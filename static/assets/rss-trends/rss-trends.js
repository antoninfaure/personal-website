fetch("../static/assets/rss-trends/data/list.json")
  .then(response => {
  ...
  .then(date_list => {
    ...
    fetch(`../static/assets/rss-trends/data/${latest_date}/edges.json`)
      .then(response => {
        if (response.status == 404) throw error;
        return response.json();
      })
      .then(links => {
        fetch("../static/assets/rss-trends/data/14-02-2023/nodes.json")
          .then(response => {
            if (response.status == 404) throw error;
            return response.json();
          })
          .then(nodes => {
            ...
            $('#dataInput').on('change', function (event) {
              ...
              if (valueSelected.match(/^(\d{1,2})-(\d{1,2})-(\d{4})$/)) {
                ...
                fetch(`../static/assets/rss-trends/data/${date}/nodes.json`)
                  .then(response => {
                    if (response.status == 404) throw error;
                    return response.json();
                  })
                  .then(new_nodes => {
                    fetch(`../static/assets/rss-trends/data/${date}/edges.json`)
                      .then(response => {
                        if (response.status == 404) throw error;
                        return response.json();
                      })
